...
SHOW_WINDOW = "DISPLAY" in os.environ
...
class QRDebug(Node):
    def __init__(self):
        ...
        self.last_saved = 0
        self.get_logger().info(f'Listening on {TOPIC}  pyzbar={"yes" if HAS_PYZBAR else "no"}  SHOW_WINDOW={SHOW_WINDOW}')
    ...
    def cb(self, msg: CompressedImage):
                ...
                for barcode in barcodes:
                    ...
                    if pts is not None and len(pts) > 0:
                        try:
                            ...
                            self.get_logger().warn(f'Failed to draw pyzbar polygon: {e}')
        ...
        if SHOW_WINDOW:
            try:
                ...
                self.get_logger().warn(f'GUI show exception: {e}')
