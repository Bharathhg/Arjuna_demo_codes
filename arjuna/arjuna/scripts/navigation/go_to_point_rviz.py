...
class ArjunaNavigator(Node):
    ...
    def goal_callback(self, msg):
        ...
        frame_id = msg.header.frame_id if msg.header.frame_id else 'odom'
        transformed_goal = self.transform_point_to_odom(msg.pose.position, frame_id)
        
        self.goal_position.x = transformed_goal.x
        self.goal_position.y = transformed_goal.y
    ...
    def transform_point_to_odom(self, point, source_frame):
        """Transform a Point from source_frame to 'odom' frame"""
        ...
        try:
            ...
            transformed_point.x = point.x * math.cos(yaw) - point.y * math.sin(yaw) + tx
            transformed_point.y = point.x * math.sin(yaw) + point.y * math.cos(yaw) + ty
            transformed_point.z = 0.0
            return transformed_point
        except Exception as ex:
            self.get_logger().warn(f"Failed to transform point from {source_frame} to odom: {ex}. Using original point.")
            return point
