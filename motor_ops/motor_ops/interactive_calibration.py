...
def run_calibration_menu(node):
    ...
    try:
        trials_input = input("Enter number of trials to run (e.g. 5 or 10): ")
        num_trials = int(trials_input.strip())
        if num_trials <= 0:
            print("Please enter a positive integer.")
            return
    except ValueError:
        print("Invalid input. Defaulting to 5 trials.")
        num_trials = 5

    try:
        rotations_input = input("Enter number of wheel rotations performed per trial (e.g. 1 or 5): ")
        rotations_per_trial = int(rotations_input.strip())
        if rotations_per_trial <= 0:
            print("Please enter a positive integer.")
            return
    except ValueError:
        print("Invalid input. Defaulting to 1 rotation per trial.")
        rotations_per_trial = 1
        
    print(f"\nWe will run {num_trials} trials. In each trial, you will rotate the wheel exactly {rotations_per_trial} times.")
    ...
    print("  [Space] EMERGENCY STOP    |  [w/x] Increase/Decrease Speed")
    ...
    for trial in range(1, num_trials + 1):
        ...
        print(f"2. Jog using [7]/[1] or [9]/[3] until wheel completes EXACTLY {rotations_per_trial} full turn(s) ({360 * rotations_per_trial}°).")
        ...
        while True:
            ...
            if key == '7':
                ...
                print("\n[EMERGENCY STOP] All motors stopped immediately.")
            elif key.lower() == 'w':
                node.MOTOR_SPEED = min(node.MOTOR_SPEED + 50, 800)
                print(f"\n[SPEED] Increased speed to {node.MOTOR_SPEED}")
            elif key.lower() == 'x':
                node.MOTOR_SPEED = max(node.MOTOR_SPEED - 50, 50)
                print(f"\n[SPEED] Decreased speed to {node.MOTOR_SPEED}")
            elif key == '5':
            ...
            sys.stdout.write(f"\rCurrent Diff -> Left: {dl:5d} ticks | Right: {dr:5d} ticks | Speed: {node.MOTOR_SPEED}")
            sys.stdout.flush()
    ...
    total_ticks_left = sum(left_trials)
    total_rotations_left = len(left_trials) * rotations_per_trial
    tpr_left = total_ticks_left / total_rotations_left if total_rotations_left > 0 else 0
    
    total_ticks_right = sum(right_trials)
    total_rotations_right = len(right_trials) * rotations_per_trial
    tpr_right = total_ticks_right / total_rotations_right if total_rotations_right > 0 else 0
    
    combined_total_ticks = total_ticks_left + total_ticks_right
    combined_total_rotations = total_rotations_left + total_rotations_right
    tpr_combined = combined_total_ticks / combined_total_rotations if combined_total_rotations > 0 else 0
    
    if left_trials:
        print("Left Wheel Stats:")
        print(f"  Total number of ticks     : {total_ticks_left}")
        print(f"  Total number of rotations : {total_rotations_left}")
        print(f"  Ticks for Rotation (TPR)  : {tpr_left:.2f}")
        print("-" * 45)
        
    if right_trials:
        print("Right Wheel Stats:")
        print(f"  Total number of ticks     : {total_ticks_right}")
        print(f"  Total number of rotations : {total_rotations_right}")
        print(f"  Ticks for Rotation (TPR)  : {tpr_right:.2f}")
        print("-" * 45)
        
    if left_trials and right_trials:
        print("Combined (Average) Stats:")
        print(f"  Total number of ticks     : {combined_total_ticks}")
        print(f"  Total number of rotations : {combined_total_rotations}")
        print(f"  Ticks for Rotation (TPR)  : {tpr_combined:.2f}")
        print("-" * 45)
